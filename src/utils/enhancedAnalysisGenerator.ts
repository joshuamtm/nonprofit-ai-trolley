import { SessionData, PathAnalysis, PersonalizationScore, MitigationStrategy } from '../types';
import { getRelevantRecommendations, generatePersonalizedRecommendations } from './recommendationTemplates';
...
export class EnhancedAnalysisGenerator {
  ...
  private calculateScores(): PersonalizationScore[] {
    ...
    // Risk Aversion Score
    const riskConcerns = ['ethicalBias', 'dataPrivacy', 'humanDignity', 'accuracyErrors'];
    const riskScore = riskConcerns.reduce((sum, concern) => {
      return sum + (this.data.primaryConcerns[concern as keyof typeof this.data.primaryConcerns] || 0);
    }, 0) / (riskConcerns.length * 5);

    scores.push({
      category: 'risk-aversion',
      weight: 0.3,
      value: riskScore,
      influence: riskScore > 0.6 ? 'High caution needed' : 'Moderate risk tolerance'
    });

    // Urgency Score
    const urgencyValue = this.data.problemUrgency === 'critical' ? 1.0 :
                        this.data.problemUrgency === 'high' ? 0.75 :
                        this.data.problemUrgency === 'moderate' ? 0.5 : 0.25;

    scores.push({
      category: 'urgency',
      weight: 0.25,
      value: urgencyValue,
      influence: urgencyValue > 0.7 ? 'Fast implementation needed' : 'Time for careful planning'
    });
    ...
    const readinessScore = readinessFactors.reduce((a, b) => a + b) / (readinessFactors.length * 5);

    scores.push({
      category: 'readiness',
      weight: 0.25,
      value: readinessScore,
      influence: readinessScore > 0.6 ? 'Well-prepared for implementation' : 'Significant preparation needed'
    });

    // Stakeholder Alignment Score
    const alignmentValue = this.data.stakeholderReadiness === 'enthusiastic' ? 1.0 :
                          this.data.stakeholderReadiness === 'supportive' ? 0.75 :
                          this.data.stakeholderReadiness === 'skeptical' ? 0.4 : 0.2;

    scores.push({
      category: 'alignment',
      weight: 0.2,
      value: alignmentValue,
      influence: alignmentValue > 0.6 ? 'Strong buy-in expected' : 'Change management critical'
    });
  ...
  private generateDetailedBenefits(): string[] {
    ...
    if (this.data.expectedOutcomes.includes('serve_more')) {
      benefits.push(`Serve ${this.data.impactScale === 'organization-wide' ? '50-75%' : '25-40%'} more beneficiaries`);
    }
  ...
  private generateDetailedRisks(): string[] {
    ...
    Object.entries(this.data.primaryConcerns).forEach(([concern, rating]) => {
      if (rating >= 4) {
        const riskMap: { [key: string]: string } = {
          ...
          environmentalImpact: 'Substantial carbon footprint from AI operations',
    ...
    if (this.data.stakeholderReadiness === 'resistant') {
      risks.push('Severe implementation challenges due to stakeholder resistance');
    }
  ...
  private generateContextualRecommendations(approach: 'aggressive' | 'cautious'): string[] {
  ...
  private generateMitigationPlaybook(): MitigationStrategy[] {
    ...
    // Get top 3 concerns
    const topConcerns = this.data.topThreeConcerns ||
  ...
  private estimateBudget(implementation: 'full' | 'phased' | 'none'): {
    ...
    const orgSize = this.data.organizationType === 'large' ? 3 :
                   this.data.organizationType === 'medium' ? 2 : 1;
  ...
  private identifyResources(implementation: 'full' | 'phased'): {
    ...
    // Tools needed
    resources.tools = implementation === 'full' ?
      ['Enterprise AI platform', 'Data management system', 'Monitoring tools', 'Training platform'] :
      ['Pilot AI tools', 'Basic analytics', 'Feedback systems', 'Documentation platform'];

    // Partnerships
    resources.partnerships = implementation === 'full' ?
      ['AI vendor', 'Technical consultant', 'Ethics advisor', 'Training provider'] :
      ['Technical advisor', 'Pilot vendor', 'Peer organizations', 'Academic partner'];
  ...
  private defineSuccessMetrics(): string[] {
    ...
    // Standard metrics
    metrics.push('Staff satisfaction score', 'Beneficiary satisfaction rate', 'Cost per service delivered', 'Error/incident rate');
  ...
  private calculateImpactScore(implementation: 'full' | 'phased' | 'none'): number {
    const baseScore = implementation === 'full' ? 85 :
                     implementation === 'phased' ? 65 : 20;

    // Adjust based on readiness
    const readinessAdjustment = this.scores.find(s => s.category === 'readiness')?.value || 0.5;
  ...
  private determineRecommendedPath(): { recommendedPath: string; rationale: string } {
    const riskScore = this.scores.find(s => s.category === 'risk-aversion')?.value || 0.5;
    const urgencyScore = this.scores.find(s => s.category === 'urgency')?.value || 0.5;
    const readinessScore = this.scores.find(s => s.category === 'readiness')?.value || 0.5;
    const alignmentScore = this.scores.find(s => s.category === 'alignment')?.value || 0.5;

    // Decision logic
    if (urgencyScore > 0.8 && readinessScore > 0.6) {
      return {
        recommendedPath: 'Pull the Lever (Full Implementation)',
        rationale: 'Your critical urgency combined with good organizational readiness suggests moving forward quickly with full implementation while managing risks actively.'
      };
    }
    ...
    if (riskScore > 0.7 || alignmentScore < 0.4) {
      if (urgencyScore < 0.5) {
        return {
          ...
          rationale: 'High risk concerns and/or low stakeholder alignment, combined with non-urgent timeline, suggest focusing on preparation before considering AI implementation.'
        };
      } else {
        return {
          recommendedPath: 'Pull with Safeguards',
          rationale: 'Your significant concerns require a careful approach, but the urgency of your needs means you should proceed with robust safeguards and phased implementation.'
        };
      }
    }
    ...
    return {
      recommendedPath: 'Pull with Safeguards',
      rationale: 'A phased approach with safeguards balances your need for AI benefits with appropriate risk management and stakeholder engagement.'
    };
  }

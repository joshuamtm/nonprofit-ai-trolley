...
import { Download, Eye, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
    ...
    switch(selectedPath) {
      ...
      case 'dont-pull':
        return {
          ...
          description: 'Maintaining current operations without AI, preserving stability while accepting limitations.',
          color: 'text-blue-600',
          bgColor: 'bg-blue-50',
      ...
      <div className="card">
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-800">How Your Inputs Shape Our Analysis</h3>
            <button
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Click on each path to see detailed analysis tailored to your organization's specific situation.
          </p>
        ...
        {pathDetails && (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className={`text-lg font-bold ${pathDetails.color} flex items-center gap-2`}>
                {pathDetails.title}
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-lg font-bold ${
                  pathDetails.analysis.impactScore >= 70 ? 'text-green-600' :
                  pathDetails.analysis.impactScore >= 40 ? 'text-yellow-600' : 'text-red-600'
                }`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {pathDetails.analysis.tradeOffSummary && (
                <>
                  <div>
                    <h4 className="font-semibold mb-2 text-green-700">What You Gain:</h4>
                  ...
                  <div>
                    <h4 className="font-semibold mb-2 text-red-700">What You Risk/Lose:</h4>
                    <ul className="text-sm space-y-1">
              ...
              <div className="mt-4 pt-4 border-t border-gray-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {pathDetails.analysis.actionPlan30Days && (
                    <div>
                      <h5 className="text-sm font-medium text-gray-600 mb-1">30 Days:</h5>
                      <ul className="text-sm space-y-1">
                        {pathDetails.analysis.actionPlan30Days.slice(0, 3).map((action: string, i: number) => (
                          <li key={i}>• {action}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {pathDetails.analysis.actionPlan60Days && (
                    <div>
                      <h5 className="text-sm font-medium text-gray-600 mb-1">60 Days:</h5>
                      <ul className="text-sm space-y-1">
                        {pathDetails.analysis.actionPlan60Days.slice(0, 3).map((action: string, i: number) => (
                          <li key={i}>• {action}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {pathDetails.analysis.actionPlan90Days && (
                    <div>
                      <h5 className="text-sm font-medium text-gray-600 mb-1">90 Days:</h5>
                      <ul className="text-sm space-y-1">
                        {pathDetails.analysis.actionPlan90Days.slice(0, 3).map((action: string, i: number) => (
                          <li key={i}>• {action}</li>
                        ))}
                      </ul>
                    </div>
            ...
            {pathDetails.analysis.budgetEstimates && (
              <div className="mt-4 p-3 bg-white bg-opacity-70 rounded">
                <h5 className="text-sm font-semibold mb-2">Budget Estimates:</h5>
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div>
                    <span className="text-gray-600">Initial:</span>
                    <p className="font-medium">{pathDetails.analysis.budgetEstimates.initial}</p>
                  </div>
                  <div>
                    <span className="text-gray-600">Ongoing:</span>
                    <p className="font-medium">{pathDetails.analysis.budgetEstimates.ongoing}</p>
                  </div>
                  <div>
                    <span className="text-gray-600">3-Year Total:</span>
                    <p className="font-medium">{pathDetails.analysis.budgetEstimates.total}</p>
                  </div>
                </div>
              </div>
            )}
